...
bool level3::init()
{
	...
	for (int i = 0; i <= 6; i++)
	{
		for (int j = 0; j <= 5; j++)
		{
			if (i == 0 || j == 0 || j == 5 || i == 6 || i == 3 && j == 3 || i == 2 && j == 1 || i == 2 && j == 2 || i == 2 && j == 3 || i == 4 && j == 3) 
			{
...
void level3::update(float delta) {
	...
	for (auto i = 0; i <= 60; i++)
	{
		for (auto j = 0; j <= 60; j++) {
			...
			{
				if ((ball->getPositionX() + i) == object->getPositionX() && (ball->getPositionY() + j) == object->getPositionY() && isnotcolored(object) == true) 
				{
					object->setColor(Color3B(255, 153, 51));
					++paint;
	...
	if (paint == 15)
	{
		for (auto object : this->theway) 
		{
			...
			auto scene = LastScene::createScene();
...
bool level3::isnotcolored(Sprite* object) {

	if (object->getColor() == Color3B(255, 153, 51)) {
		return false;
	}
	return true;
}